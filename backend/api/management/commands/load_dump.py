...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        dump_file_path = os.path.join(os.path.dirname(__file__),
                                      "../../../dump.sql")
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    "Successfully loaded dump.sql into the database"
                )
            )
        except Exception as e:
            self.stderr.write(self.style.ERROR(f"Error loading dump.sql: {e}"))
